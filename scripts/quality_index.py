...
def _test_evidence(relative: str, language: str, all_source_paths: Sequence[str]) -> list[str]:
...
def _documentation_evidence(
    relative: str, path: Path, documentation: Mapping[str, str], root: Path
) -> dict[str, Any]:
    ...
    for doc_path, text in documentation.items():
        if exact in text or (len(basename) >= 8 and basename in text):
            evidence.append(doc_path)
    unique = sorted(set(evidence))
    return {
        "status": "evidence" if unique else "gap",
        "evidence": unique,
        "method": "heuristic-path-or-in-source-docstring",
    }
...
def build_index(
    ...
    *,
    ...
    for relative in source_paths:
        ...
        tests = _test_evidence(relative, language, source_paths)
        docs = _documentation_evidence(relative, path, documentation, root)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        return 1
    print(json.dumps({"ok": True, "json": str(json_path), "markdown": str(markdown_path), "summary": index["summary"]}, sort_keys=True))
    return 0
