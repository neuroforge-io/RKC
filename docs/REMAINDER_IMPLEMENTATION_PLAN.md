...
| 2026-08-30 | Confidential resource-admission diagnostics | Shell and Python priority guards retain raw process arguments only long enough to classify admission, then expose only PID/fixed-class receipts; the isolation doctor discards user-manager environment output. Secret-sentinel tests cover the CLI, Python, and shell paths | Normalize credentialed repository origins before identity derivation; make privacy modes enforceable; close remaining bounded-read, workbench-origin, remote-bind, plaintext-Git, and first-run resource-admission gaps |
| 2026-08-30 | Typed GUI command catalogue and workbench bootstrap | One Go catalogue now supplies the generated static browser and live workbench with valid command/subcommand vectors, guidance, and exact file-atlas or immutable SQLite snapshot selectors; paths remain arrays through rendering, and cross-origin browser requests cannot obtain the workbench token | Replace raw-argument-first UX with guided fields, add live output/recovery and large-atlas paging, and establish reproducible browser, accessibility, mobile, macOS, and Windows acceptance gates |
