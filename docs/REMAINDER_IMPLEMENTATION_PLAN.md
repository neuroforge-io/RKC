...
| 2026-07-23 | P2 deterministic scan DAG and verified analyzer cache slice | All 15 canonical scan stages execute through the scheduler with bounded CPU/memory/process/open-file admission; analyzer stages emit isolated, ownership-bound fragments into verified CAS payloads; dynamic per-stage keys retain unaffected Go/TypeScript/framework work; rejected payloads recompute safely; `rkc plan` and cache inspect/verify/prune expose behavior; focused cold/warm/localized-change tests prove canonical equality with a clean scan | Persist stage journal and ownership metadata in SQLite, add process-kill recovery plus maintained multi-commit history fixtures, and measure the warm-scan exit gate before closing P2 |
| 2026-07-27 | P2 scheduler and command run-journal slice | Every CLI scan reaching DAG execution creates a new owner-only append-only journal outside the repository, binds it to the scheduler, defers terminal success until policy and configured publications complete, reports its ID/path, and exposes bounded resilient `runs list/show` inspection; schema, sequence, plan/lifecycle, and hash-chain checks fail closed while a crash-truncated tail retains its validated prefix; Unix permissions are runtime-tested and a protected current-user Windows DACL cross-compiles | Run the native Windows ACL gate; add explicit retention/pruning, SQLite ownership persistence, leases/retries and process-kill restart fixtures; convert acquisition and derived outputs into stages; retain clean/incremental equivalence and warm-scan gates |
| 2026-08-30 | Quality, usability, and attribution slice | `rkc open`/`start` provides a non-technical scan/check/serve flow; NotebookLM packs include a bounded upload guide and deterministic source manifest; `.rkc-quality/index.json` and `index.md` inventory source hashes, test/documentation evidence, optional Go/Python profiles, and Git deltas; the then-current surfaces declared NeuroForgeIO stewardship and MIT notice terms while keeping third-party/model terms separate; that project-license posture is superseded by the 2026-08-31 Apache-2.0 conversion below; CI exercises the index and uploads the report | File-level evidence is intentionally heuristic and currently exposes gaps rather than claiming semantic 100%; close remaining gaps by adding tests/docs/profiles and keep release thresholds/provenance/signing/team-service boundaries governed in their existing rows |
| 2026-08-31 | Apache-2.0 ownership and distribution boundary | Canonical Apache-2.0 text, NeuroForgeIO copyright/NOTICE, API/plugin/container/package/SBOM metadata, first-party documentation, generated atlas/report branding, and fail-closed release validation consistently identify original RKC materials as copyright NeuroForgeIO under Apache-2.0 | Third-party dependencies, optional runtimes, model assets, analyzed repositories, and generated repository content retain their own ownership and licenses; future additions must extend the reviewed notice and SBOM boundary rather than inherit RKC ownership |
| 2026-08-30 | Quality-index and example-profile hardening | Header-only generated-file detection removes runtime-string false positives; Go package comments and explicit cross-language harness references are indexed; shell workflow syntax contracts, output-ancestor symlink rejection, strict `--fail-on-gaps`, Go zero-statement/platform metadata, Python example branch coverage, and the nested Go example profile are wired into CI | Semantic behavior coverage, non-Linux native execution, signed provenance, and the team-service boundary remain governed by the release gates below; keep the profile artifact fresh on every release |
| 2026-08-30 | Quality-index profile accountability | Partial executable profiles now create per-file uncovered-unit gaps, and malformed/unowned profile evidence fails closed instead of presenting a misleading complete report | The index remains an evidence/triage surface; measured release thresholds and semantic review remain authoritative |
| 2026-08-30 | Exact coverage-gap localisation | The quality index now deduplicates repeated Go blocks with covered-OR semantics, rejects conflicting denominators/coordinates and inconsistent branch-aware Python counters, and retains exact uncovered Go regions plus Python lines/branch arcs in its machine report | Use the fresh current-SHA CI artifact to close the highest-risk behavior gaps; exact locations improve triage but do not by themselves prove semantic test quality |
| 2026-08-30 | Confidential resource-admission diagnostics | The shell priority guard requests PID-only results and classifies by its fixed query, while the Go and Python guards keep any required process inspection private and expose only PID/fixed-class receipts; the isolation doctor discards user-manager environment output. Secret-sentinel and record-injection tests cover the CLI, Python, and shell paths | Retain fixed-output regression gates as process admission evolves; diagnostics must never reproduce another workload's arguments, prompts, paths, or environment |
| 2026-08-30 | Typed GUI command catalogue and workbench bootstrap | One Go catalogue now supplies the generated static browser and live workbench with valid command/subcommand vectors, guidance, and exact file-atlas or immutable SQLite snapshot selectors; paths remain arrays through rendering, and cross-origin browser requests cannot obtain the workbench token. A later hardening slice now delivers the bootstrap as a one-time URL-fragment capability through an owner-private readiness receipt, removes the fragment before exchange, and invalidates it after first use. The 2026-08-31 activation closure validates exact GUI `quickstart <folder>` output through the canonical dataset loader and atomically retargets every read view and dataset-aware command only after snapshot, ownership, export-manifest, and root identity agree; corrupt output leaves the old atlas active | Replace remaining raw-argument-first workflows with guided fields, add live output/recovery and large-atlas paging, and establish reproducible browser, accessibility, mobile, macOS, and Windows acceptance gates |
| 2026-08-30 | Canonical repository-origin confidentiality boundary | Acquisition retains raw Git operands only for the child invocation, rejects plaintext or secret-bearing URL forms, denies unapproved Git protocols, and exposes one credential-free canonical origin. Staged/sequential scans reconcile supplied and configured origins, derive stable IDs only from that value, always exclude `.git`, validate cross-field provenance, and reject unsafe export before any output is created. Credential-only variants are regression-tested for identical identity and digest | Bind and verify SQLite `repositories.canonical_origin` transactionally, enforce the same affinity in the in-memory conformance store, and add complete CLI/store/API sentinel traversal before closing the persistence audit |
| 2026-08-30 | Privacy-safe durable repository affinity | SQLite schema 0.5 renames the unused plaintext-origin slot to `repository_affinity` and binds only the already-public opaque RepositoryID; memory and SQLite stores enforce the same atomic affinity contract across canonical-to-redacted and redacted-to-canonical histories. Migration preflight rejects noncanonical, credential-bearing, or cross-field-inconsistent legacy provenance without disclosure, and every public store reader/search surface fails closed on affinity corruption. Real CLI scans prove redacted atlas, SQLite, WAL, and SHM bytes omit origin and absolute-path sentinels | Retain explicit HTTP/MCP response sentinel traversal in the release integration suite; the storage and CLI publication boundary itself is closed and no durable affinity requires plaintext origin retention |
| 2026-08-30 | Protected installed first run | On Linux the installed `rkc open` binary self-reexecutes with the same working directory and a deterministic allowlisted environment into the exact one-core, nice-19, idle-I/O, 4 GiB pressure / 4.5 GiB hard-memory service before scan-side state exists. Grammar-aware help detection cannot be confused by flag values or `--`; the outer monitor continuously pre-empts for ERAIS, proves cleanup, preserves XDG locations, and launches the browser outside the service. The child publishes a bounded owner-private readiness receipt containing a one-time URL-fragment capability; the outer process validates it, launches a private redirect, and the browser strips and consumes the fragment exactly once. Every live server regenerates UI bytes from the current binary; workbench responses are non-cacheable, CSP denies new workers/manifests/forms, and the OS selects an ephemeral loopback port to reduce deliberate origin reuse without claiming the port was never used before. Direct `serve --workbench` requires `--ready-file` and rejects `--open`. Read-only remains the portable default and `--workbench` is explicit Linux-only | Before default-on workbench promotion, close workspace path confinement, nested-model aggregate ceiling, guided forms/live output/large-atlas paging, browser/assistive-technology gates, and equivalent macOS/Windows native admission |
| 2026-08-30 | Dependency-free guided terminal first run | `rkc wizard`/`tui` now prompts for an arbitrary folder and composes the existing safe `open` or compile-only `quickstart` service directly without a shell. A bounded line reader, explicit cancel/help routes, clean EOF behavior, injected action seams, full focused statement coverage, real dispatcher/catalogue checks, and installer/quickstart/status documentation keep the feature portable and testable. The guide explicitly does not claim full CLI parity | Retain the line-oriented portable guide as a small first-run surface; add options only when they remove a demonstrated usability barrier without duplicating the command framework or bypassing existing admission and quality gates |
| 2026-08-30 | Self-atlas live-load efficiency | The verifier still inventories and hashes all 7,840 files in the measured RKC self-atlas, but now reuses one bounded copy buffer, retains only the three canonical inputs consumed by the live server, rebuilds executable browser assets from the current binary, and checks decoded canonical form with an O(n), O(1)-auxiliary scan. On the 62,585,787-byte bundle, cumulative allocation fell from 1,727.15 MiB to 1,031.66 MiB (40.27%); a separate one-core lowest-priority run completed in 2.90 seconds at 612,504 KiB maximum RSS without swap | Retain the exact integrity and noncanonical-input regressions. Large-atlas paging/streaming remains governed by the existing browser and storage rows; do not trade away complete hash or coverage validation for headline startup numbers |
| 2026-08-30 | Exported Go documentation accountability | The quality index now uses a standard-library Go AST helper with downloads disabled to report attached documentation separately from file-level heuristics, including exact missing symbol/kind/line/column records and fail-closed parser/tool errors. Integrator-facing `pkg/client`, `pkg/pluginapi`, `pkg/graphpatch`, `pkg/rkcmodel`, and `pkg/rkcstore` are 206/206; the full production tree is 350/773 (45.28%) | The remaining 423 command/internal/example declarations stay visible in the generated queue. Improve them alongside behavior work where the comment communicates a durable contract; do not add filler merely to manufacture a whole-tree percentage |
...
The future producer-authenticated collector must support observed calls,
coverage, loaded plugins/modules, registered routes, configuration reads, SQL
queries, and emitted/consumed events. Every authenticated observation must
include environment and run context. The current reference imports only
integrity-protected operator assertions; it does not claim those event streams.
Lack of observation never becomes proof of absence.
...
## 2026-08-31 causal executable-model extension

The product target is a bounded causal and executable model of a software
system, not a larger keyword index. This section is additive to every issue and
gate above.

| Layer | Implemented evidence | Next executable closure |
|---|---|---|
| Compiler semantics | Strict streaming SCIP import/generation/verification with exact definitions, references, relationships, diagnostics, and source encodings | Maintained compiler adapters and fixtures for each claimed language; unresolved-rate budgets by language/toolchain/configuration |
| Control and data flow | Bounded Go call graph, per-function CFG, value origins/sinks, non-authoritative sanitizer-name hypotheses, parameter/return binding, and evidence-linked lineage | Compiler/SSA authority, predicate conditions, mutation/alias/heap flow, proven sanitizer semantics, interprocedural summaries, and equivalent adapters beyond Go |
| Runtime actuality | Guarded Go-test capture, source-bound covered-span assertions, terminal result assertions, multi-trace union, and explicit static-versus-asserted reports; all current imports remain `user_asserted` at confidence 0.5 | Producer-authenticated capture and compile for CLI/GUI; transient-mutation isolation; temporal traces with per-test/process identity, typed events, branch/value observations, configuration receipts, and reproducible observation manifests. Process locality and a self-hash are never sufficient authentication |
| Configuration semantics | Environment templates/reads, build tags, CI structure, package/Docker manifests, Terraform declarations, OpenAPI and schema facts | Resolve dependency injection, feature-flag predicates, deployment overlays, migrations, event schemas, and code/config reachability into one conditional graph |
| Tests as specifications | Package-qualified result evidence and coverage linkage | Extract setup → stimulus → assertions → expected response/side effects as machine-readable, cited behavioural contracts without executing tests during ordinary scans |
| Semantic history | Bounded first-parent Go/TypeScript symbol additions, removals, signature changes, moves, lifecycles, and conservative supersedes edges | Co-change topology, failure/test correlation, responsibility/interface evolution, release boundaries, and additional language extractors |
| Federated systems | Versioned export, HTTP, MCP, SQLite, GraphML and schema contracts make snapshots composable | Cross-repository identities for API clients, protobufs, events, queues, services, deployments and databases, with tenant/privacy/pagination boundaries |
| Active evidence | `rkc plan` validates supplied SCIP/trace/history inputs and emits explicit authorization-bearing opportunities without executing them | Canonical uncertainty/request/attempt/result records and a bounded question-driven acquire → validate → recompile → reason loop |
| Counterfactuals | Immutable baseline-versus-omission route comparison with exact interventions, evidence, bounds, and non-authoritative status | Predicate-, configuration-, runtime- and history-conditioned scenarios with explicit uncertainty; no model may rewrite canonical facts |
| Learned semantics | Qualified embeddings and grounded evidence packets are derived and fail closed when no model passes | Latent subsystem hypotheses, concept clusters and agent explanations that always cite deterministic evidence and remain outside source truth |

Every new terminal capability must also have a bounded graphical workflow and a
machine-readable interface. GUI parity is measured by validated outcomes and
snapshot identity—not by placing an arbitrary command text box in a browser.
RKC remains agent-neutral and has no dependency on ERAIS; any reasoning system
uses the same public evidence contracts as a human or another application.

Additional tracked closures:

86. `SEM-CAUSAL-001`: compiler/SSA interprocedural summaries and uncertainty.
87. `RUNTIME-TRACE-001`: temporal, per-test, configuration-bound observations.
88. `TEST-SPEC-001`: cited behavioural-contract extraction.
89. `FEDERATION-001`: cross-repository/service identity and contract graph.
90. `ACQUIRE-001`: canonical active-evidence request lifecycle.
91. `COUNTERFACTUAL-001`: conditional counterfactual semantics above the truth layer.
92. `GUI-CAUSAL-001`: outcome-proven graphical parity for every safe causal surface.
