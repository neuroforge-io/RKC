...
| Model default | Intentionally unset after complete qualification | Granite 4 H 1B Q5_K_M was the strongest plausible Apache-2.0 candidate but passed only 2/6 generation cases, emitted the injection canary, made unsupported claims, and processed only about 3,072 of 32,384 long-context prompt tokens before the five-minute deadline. Gemma 4 E2B failed the sampler/long-context path; Qwen3.5 2B was CPU-ineffective at 32K; Qwen3.5 0.8B also passed only 2/6 cases and timed out. The paired Qwen3 embedding role passed all gates, but pair-level promotion correctly remained disabled |
...
| cgroup, priority, CPU-only and RSS policy | Implemented on Linux; fail-closed elsewhere | Linux cgroup v2 enforces one CPU, weight 1, nice 19, idle I/O, 4 GiB operating/4.5 GiB hard memory, bounded swap/tasks, CPU-only runtime flags, ERAIS pre-emption, process-group reap, and auditable receipts. Heavy model operations refuse platforms that cannot prove this contract |
...
| real GGUF benchmark below 4.5 GiB | Qualification in progress; no promotion | Granite 4 H 1B Q5_K_M stayed below the former protected ceiling but passed only 2/6 generation cases, emitted the injection canary, and reached roughly 3,072/32,384 prompt tokens in five minutes at about 12 tokens/second. Qwen3.5 0.8B Q4_0 used only 1.55 GB of protected-cgroup memory and ran faster, but also passed only 2/6 cases and timed out at exact 32K. Qwen3 Embedding 0.6B Q8_0 passed recall, margin, norm, memory, and latency checks. Both required pairs failed, so defaults remain null while larger Qwen candidates are evaluated under the new envelope |
