...
| HTTP listener confidentiality | Implemented | Loopback is the fail-closed default; non-loopback read-only serving requires explicit `--allow-remote`, the workbench remains loopback-only, and API responses use `private, no-store` plus same-origin resource policy |
