...
func normalizeLlamaCPPEmbeddingConfig(config LlamaCPPEmbeddingConfig) (LlamaCPPEmbeddingConfig, error) {
	...
	if config.MaximumRSSBytes == 0 {
		config.MaximumRSSBytes = 4608 * 1024 * 1024
	}
	...
	if !config.UnsafeDisableResourceGuard && config.MaximumRSSBytes > 4608*1024*1024 {
		return config, errors.New("production embedding RSS limit must not exceed 4608 MiB")
	}
	return config, nil
}
