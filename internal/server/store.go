...
// PrepareWorkbenchBrowser replaces every persisted browser byte with the
// application shell generated by the current RKC binary. Export-manifest hashes
// establish data integrity, not publisher authenticity, so imported site
// JavaScript is never executable on the same origin as the privileged local
// workbench API. Full atlas data remains available through the bounded API.
