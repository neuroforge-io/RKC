...
// PrepareWorkbenchBrowser replaces every persisted browser byte with assets
// generated by the current RKC binary from the already-validated canonical
// bundle and coverage. Export-manifest hashes establish data integrity, not
// publisher authenticity, so imported site JavaScript is never executable on
// the same origin as the privileged local workbench API.
func (dataset *Dataset) PrepareWorkbenchBrowser() error {
	if dataset == nil {
		return errors.New("prepare workbench browser: dataset is required")
	}
	if dataset.staticSiteTrusted {
		return nil
	}
	browserAssets, err := rkcexport.BrowserAssets(dataset.Bundle, dataset.Coverage)
	if err != nil {
		return fmt.Errorf("prepare workbench browser: %w", err)
	}
	staticSite, err := captureGeneratedBrowser(browserAssets)
	if err != nil {
		return fmt.Errorf("prepare workbench browser: %w", err)
	}
	dataset.staticSite = staticSite
	dataset.staticSiteTrusted = true
	return nil
}
